...
if (prueba){
  ...
  salida <- bajarCopernico(       ruta.ftp = 'ftp://gramirez2:$boletinDHN2018@nrt.cmems-du.eu/Core/SST_GLO_SST_L4_NRT_OBSERVATIONS_010_001/METOFFICE-GLO-SST-L4-NRT-OBS-ANOM-V2/'
                                  ,   ruta.salida = raiz
...
png(filename = paste0(boletin.dir,'mapa_costero_tsm_mensual', fecha.actual ,'.png'),
    width = 1200,
    height = 1200)
plot(mapa_costero)
dev.off()
...
png(filename = paste0(boletin.dir,'mapa_costero_atsm_mensual', fecha.actual ,'.png'),
...
png(filename = paste0(boletin.dir,'mapa_ecua_atsm_mensual', fecha.actual ,'.png'),
...
if (diario){
  #################################################
  archivo.hoy <- last(archivos)
  ######## MAPA COSTERO DE TEMPERATURA DIARIO ######
  source('boletin_mapa_costero_temp.R')
  lim.lon <- c(270, 290)
  lim.lat <- c(-20, 2)
  # archivo.hoy <- last(archivos)
  
  mapa_costero <- mapa_costero_temp(archivo.hoy,'analysed_sst',
                                    lim.lon,lim.lat,
                                    fecha.actual,fecha.actual)
  
  png(filename = paste0(boletin.dir,'mapa_costero_tsm', fecha.actual ,'.png'),
      width = 1200,
      height = 1200)
  plot(mapa_costero)
  dev.off()
  
  ######## MAPA COSTERO DE ANOMALÍA DE TEMPERATURA DIARIO#######
  
  source('boletin_mapa_costero_temp.R')
  lim.lon <- c(270, 290)
  lim.lat <- c(-20, 2)
  mapa_costero <- mapa_costero_temp(archivo.hoy,'sst_anomaly',
                                    lim.lon,lim.lat,
                                    fecha.actual,fecha.actual)
  
  png(filename = paste0(boletin.dir,'mapa_costero_atsm', fecha.actual ,'.png'),
      width=1200,height=1200)
  plot(mapa_costero)
  dev.off()
  ####### MAPA ECUATORIAL DE ANOMALÍA DE TEMPERATURA DIARIO #######
  source('boletin_mapa_costero_temp.R')
  if (exists('mapa_costero')) rm(mapa_costero)
  lim.lon <- c(120,290)
  lim.lat <- c(-25, 25)
  ...
  png(filename = paste0(boletin.dir,'mapa_ecua_atsm_', fecha.actual ,'.png'),
      width=2400,height=850)
  plot(mapa_costero)
  dev.off()}
...
###################################
   fecha.actual <- lubridate::today()
 fecha.anterior <- as.Date('2019-11-20') #fecha.actual - dias.atras # en días
...
lim.lat <- c(-20, 2)
hovmoller_costero <- hovmoller_costero(lista=archivos,poligono=poli50,'sst_anomaly',lim.lat,'50')
...
T <- as.numeric(levels(factor(hovmoller_costero$data$tiempo)))

tiempos <- split(hovmoller_costero$data,
                 cut(hovmoller_costero$data$tiempo,breaks = as.numeric(levels(factor(hovmoller_costero$data$tiempo)))))
...
png(filename = paste0(boletin.dir,'promedio_atsm_50millas_',fecha.actual,'.png'),
...
source('boletin_hovmoller_costero_temp.R')
hovmoller_costero <- hovmoller_costero(archivos,poli50,'analysed_sst',lim.lat,'50')
...
pp <- pp + geom_line(col='blue',size=2)
pp <- pp + scale_x_datetime(date_breaks = 'month',date_labels = '%b-%Y' )
pp <- pp + scale_y_continuous(limits = c(15,32) )
pp <- pp + labs( x='Fecha',y='ATSM (°C)',
