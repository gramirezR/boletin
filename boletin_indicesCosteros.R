...
################### INDICE DE ATSM COSTERO ###################
 ...
 zona <- factor(c( rep('norte', length(promediosNorte)),
  ...
  pp <- pp + scale_y_continuous(limits = c(-4,4), breaks =  seq(-4,4,by=0.5))
  pp <- pp + labs(x='Fecha',y='Anomalía',
                  title='Anomalías diarias de la TSM dentro de las 50 millas',
  ...
  # pp <- pp + labs(x='Fecha',y='Índice',
  #                 title='Índice costero de la TSM dentro de las 50 millas',
