...
arch <- lista.archs[1]
ncin <- nc_open(arch)
u <- 0.0*ncvar_get(ncin,'eastward_wind')
v <- 0.0*ncvar_get(ncin,'northward_wind')
...
suave <- gam(mag ~ te(tiempo,lat,k=c(50,20)),data = interpolar)
 pred <- predict(suave,newdata = as.data.frame(malla))
    ...
    suave <- gam(v ~ te(tiempo,lat,k=c(50,20)),data = interpolar)
    pred <- predict(suave,newdata = as.data.frame(malla))
    V <- stack( pred )$values
...
hovmoller <- data.frame(   tiempo = as.Date(malla$tiempo,origin='1970-01-01'),
                           ...
                           u = U,
                           v = V)
hovmoller$u <- hovmoller$u/hovmoller$mag
hovmoller$v <- hovmoller$v/hovmoller$mag
